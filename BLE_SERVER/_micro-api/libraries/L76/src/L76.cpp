...
boolean I2CGPS::begin(TwoWire &wirePort, uint32_t i2cSpeed, uint8_t sdapin, uint8_t sclpin)
{
	//Bring in the user's choices
	_i2cPort = &wirePort; //Grab which port the user wants us to use
	_i2cPort->begin(sdapin, sclpin, i2cSpeed);

	_head = 0; //Reset the location holder
	_tail = 0;

	//Ping the module to see if it responds
	_i2cPort->beginTransmission(L76_ADDR);
	_i2cPort->write(0); //Write dummy value
	_i2cPort->endTransmission();

	if (_i2cPort->requestFrom(L76_ADDR, 1))
		return (true); //Success!
	else
...
void I2CGPS::check()
{
	...
	for (uint8_t x = 0; x < MAX_PACKET_SIZE; x++)
	{
		if (x % 32 == 0) //Arduino can only Wire.read() in 32 byte chunks. Yay.
			_i2cPort->requestFrom(L76_ADDR, 32); //Request 32 more bytes
