...
#if defined(ESP32)
	uint8_t mosi;                  // byte 9: pin for master out / slave in (write to LORA chip)
	uint8_t miso;                  // byte 10: pin for master in / slave out (read from LORA chip)
	uint8_t sck;                   // byte 11: pin for serial clock by master
#endif
